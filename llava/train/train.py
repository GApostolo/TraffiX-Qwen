...
"""
LLaVA Training Script

This module contains the training pipeline for LLaVA (Large Language and Vision Assistant) models,
including data preprocessing, model initialization, and training loop management.
"""
...
class ModelArguments:
    """Arguments for model configuration and initialization."""
    ...
    mm_tunable_parts: Optional[str] = field(
        default=None, 
        metadata={"help": 'Could be "mm_mlp_adapter", "mm_vision_resampler", "mm_vision_tower,mm_mlp_adapter,mm_language_model", "mm_vision_tower,mm_mlp_adapter,mm_language_model", "mm_mlp_adapter,mm_language_model"'}
    )
    ...
    graph_input_dim: Optional[int] = field(default=None)
    graph_hidden_size: Optional[int] = field(default=None)

    video_token_selection: Optional[str] = field(default=None)
...
class DataArguments:
    ...
    video_frame_sampling_strategy: str = field(
        default="uniform", 
        metadata={"help": "How to sample frames from video."}
    )
...
def maybe_zero_3(param, ignore_status=False, name=None):
    """Extract parameter data from DeepSpeed Zero-3 optimization."""
...
def get_peft_state_maybe_zero_3(named_params, bias):
    """Get PEFT state dictionary with DeepSpeed Zero-3 support."""
    if bias == "none":
...
def get_peft_state_non_lora_maybe_zero_3(named_params, require_grad_only=True):
    """Get non-LoRA state dictionary with DeepSpeed Zero-3 support."""
...
def get_mm_adapter_state_maybe_zero_3(named_params, keys_to_match):
    """Get multimodal adapter state dictionary with DeepSpeed Zero-3 support."""
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ["mm_projector", "vision_tower", "graph_tower", "vision_resampler"]
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer, output_dir: str):
    """Safely save model state for HuggingFace trainer with adapter support."""
...
def smart_tokenizer_and_embedding_resize(
    ...
    """Resize tokenizer and embedding with proper initialization."""
...
def _mask_targets(target, tokenized_lens, speakers):
    """Mask targets based on speaker roles."""
...
def preprocess_llama_2(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    """Preprocess data for LLaMA-2 model format."""
...
def preprocess_gemma(sources: List[List[Dict[str, str]]], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    """Preprocess data for Gemma model format."""
...
def parse_tuples(entry):
    """Parse coordinate tuples from a given entry string to extract key frames."""
...
def preprocess_qwen_with_keyframes(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    """Preprocess data for Qwen model with key frame extraction."""
    roles = {"human": "user", "gpt": "assistant"}
    conversation = conversation_lib.default_conversation.copy()
    system_message = "You are an AI assistant specializing in the analysis of traffic scenes from surveillance footage. Each object's position at a specific moment in the video is represented as a tuple: (id, nf, x, y), where id is the unique identifier for the object, nf is the normalized timestamp of the frame (a float between 0 and 1), and x and y are the normalized coordinates (also between 0 and 1) of the object's position within that frame. Provide precise and informed responses to the questions."

    # Extract key frames from questions
    key_frames = []
    for source in sources:
    ...
    # Add image tokens to tokenizer as special tokens
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    """Preprocess data for Qwen model format without key frames."""
    roles = {"human": "user", "gpt": "assistant"}
...
def preprocess_llama3(
    ...
    """Preprocess data for LLaMA-3 model format."""
    roles = {"human": "user", "gpt": "assistant"}
    ...
    def safe_tokenizer_llama3(text):
        """Safely tokenize text for LLaMA-3, handling BOS token."""
...
def preprocess_mpt(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    """Preprocess data for MPT model format."""
    conv = conversation_lib.default_conversation.copy()
    roles = {"human": conv.roles[0], "gpt": conv.roles[1]}
...
def preprocess_plain(
    ...
    """Preprocess data for plain format."""
    conversations = []
    for source in sources:
        assert len(source) == 2
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    """Main preprocessing function that routes to appropriate format handler."""
    ...
    if conversation_lib.default_conversation.version == "qwen_traffic":
        return preprocess_qwen_with_keyframes(sources, tokenizer, has_image=has_image)
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments, model):
        super(LazySupervisedDataset, self).__init__()
        self.tokenizer = tokenizer
        self.vision_encoder = model.get_model().get_vision_tower()
        self.list_data_dict = []
        self.video_list = []
        # breakpoint()
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            try:
                ...
                processor = self.data_args.image_processor
                image = processor.preprocess(video, return_tensors="pt")["pixel_values"] # image torch.Size([23, 3, 384, 384])
        ...
        if isinstance(i, int):
            if "key_frames" in data_dict:
                data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0], key_frames=data_dict["key_frames"][0])
            else:
                data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0], key_frames=[])
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # Handle key_frames if present
        has_key_frames = "key_frames" in instances[0]
        if has_key_frames:
            key_frames = [instance["key_frames"] for instance in instances]
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args, model) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    train_dataset = LazySupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path, data_args=data_args, model=model)
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if any(
        [
            ...
            model_args.graph_tower is not None,
            model_args.graph_input_dim is not None,
            model_args.graph_hidden_size is not None
        ]
    ):
        cfg_pretrained = AutoConfig.from_pretrained(model_args.model_name_or_path)

    if model_args.graph_tower is not None or model_args.graph_input_dim is not None or model_args.graph_hidden_size is not None:
        overwrite_config["graph_tower"] = model_args.graph_tower
        overwrite_config["graph_input_dim"] = model_args.graph_input_dim
        overwrite_config["graph_hidden_size"] = model_args.graph_hidden_size

    if model_args.video_token_selection is not None:
        overwrite_config["video_token_selection"] = model_args.video_token_selection
...
def train(attn_implementation=None):
    ...
    if model_args.graph_tower is not None:
        ...
        graph_tower.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.video_token_selection = model_args.video_token_selection
            ...
            if "mm_vision_tower" in tunable_parts:
                for name, param in model.named_parameters():
                    if "vision_tower" in name:
                        # breakpoint()
                        param.requires_grad_(True)
            if "mm_graph_tower" in tunable_parts:
                for name, param in model.named_parameters():
                    if "graph_tower" in name:
                        # breakpoint()
                        param.requires_grad_(True)
    ...
    data_module = make_supervised_data_module(tokenizer=tokenizer, data_args=data_args, model=model)
